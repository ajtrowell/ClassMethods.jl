...
function capture_include_error(src::AbstractString)
    try
...
module ShortFormDocExample
using StructMethods: @structmethods

@structmethods mutable struct Cat
    value::Int

    """Short form doc for purr"""
    purr(self::Cat) = self.value
end

end # module ShortFormDocExample
...
module CustomShowExample
using StructMethods: @structmethods

@structmethods mutable struct Fish
    name::String
    age::Int
end

function Base.show(io::IO, ::MIME"text/plain", fish::Fish)
    print(io, "Fish($(fish.name))")
end

end # module CustomShowExample
...
@testset "StructMethods" begin
    @testset "Constructors and method forwarding" begin
    ...
    @testset "Docstrings" begin
        ...
        dog_years_text = join(String.(dog_years_doc.text))
        @test occursin("Return the age in dog years", dog_years_text)

        short_meta = meta(ShortFormDocExample)
        purr_binding = Binding(ShortFormDocExample, :purr)
        @test haskey(short_meta, purr_binding)
        purr_doc = first(values(short_meta[purr_binding].docs))
        purr_text = join(String.(purr_doc.text))
        @test occursin("Short form doc for purr", purr_text)
    end

    @testset "Typed docstring formatting" begin
    ...
    @testset "Closure syntax methods" begin
    ...
    @testset "Const method fields" begin
        macro_based = MacroExample.MyClass(10)

        @test getfield(macro_based, :describe) isa Function
        assignment_error = try
            macro_based.describe = identity
            nothing
        catch err
            err
        end
        @test assignment_error isa ErrorException
        @test occursin("const field", assignment_error.msg)
    end

    @testset "Show behaviour" begin
        vanilla = VanillaExample.MyClass(10)
        macro_based = MacroExample.MyClass(10)
        ...
        custom = CustomShowExample.Fish("Nemo", 2)
        io = IOBuffer()
        show(io, MIME"text/plain"(), custom)
        @test String(take!(io)) == "Fish(Nemo)"
    end

    @testset "Error handling" begin
        inner_error = capture_include_error("""
module InnerConstructorFailure
using StructMethods: @structmethods

@structmethods mutable struct Bad
    value::Int
...
""")
        @test inner_error isa ErrorException
        @test occursin("does not support manually defined inner constructors", sprint(showerror, inner_error))

        duplicate_error = capture_include_error("""
module DuplicateMethodFailure
using StructMethods: @structmethods

@structmethods mutable struct Dup
    value::Int

    function foo(self::Dup)
        self
    end

    function foo(self::Dup, x::Int)
        self
    end
end
end
""")
        @test duplicate_error isa ErrorException
        @test occursin("Duplicate method definition for foo", sprint(showerror, duplicate_error))

        no_field_error = capture_include_error("""
module NoFieldFailure
using StructMethods: @structmethods

@structmethods mutable struct NoFields
    function foo(self::NoFields)
        nothing
    end
end
end
""")
        @test no_field_error isa ErrorException
        @test occursin("requires at least one non-method field", sprint(showerror, no_field_error))
    end
end
